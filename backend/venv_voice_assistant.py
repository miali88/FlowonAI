...
class VoiceAssistant(utils.EventEmitter[EventTypes]):
    ...
    def _update_state(self, state: AgentState, delay: float = 0.0):
        ...
        async def _run_task(delay: float) -> None:
            ...
            # Log the full state data
            logger.info(f"Call state updated: {state_data}", extra={"job_id": self._job_id})

            # Send state change to backend
            async with aiohttp.ClientSession() as session:
                try:
                    async with session.post(f'{DOMAIN}/voice/state', json=state_data) as response:
                        if response.status == 200:
                            logger.debug("Successfully sent state change to backend", extra={"job_id": self._job_id})
                        else:
                            logger.warning(f"Failed to send state change to backend. Status: {response.status}", extra={"job_id": self._job_id})
                except Exception as e:
                    logger.error(f"Error sending state change to backend: {str(e)}", extra={"job_id": self._job_id})
    ...
    def entrypoint(ctx):
        job_id = ctx.job.id  # Correctly access the job ID
        print(f"Entrypoint called with job_id: {job_id}")
        # Initialize your VoiceAssistant instance
        assistant = VoiceAssistant(...)
        assistant.set_job_id(job_id)
