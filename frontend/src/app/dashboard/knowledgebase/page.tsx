...
function KnowledgeBaseContent() {
  ...
  const fetchUserSpecificData = useCallback(async () => {
    ...
    try {
      ...
      setAlertMessage(
        "Failed to fetch user data: " + (apiError.message || "Unknown error")
      );
      setAlertType("error");
    } finally {
      setIsLoading(false);
  ...
  const handleFileUpload = async () => {
    if (!selectedFile || !user) {
      setAlertMessage(
        selectedFile ? "User not authenticated" : "No file selected"
      );
      setAlertType("error");
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/knowledge_base/upload_file`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setAlertMessage(
        "File processed and added to Knowledge Base successfully"
      );
    ...
    <div className="flex flex-col h-full">
      ...
      >
        <TabsList className="w-full justify-start">
          <TabsTrigger value="insert" className="text-sm">
            Insert
          </TabsTrigger>
      ...
      <div className="flex h-full justify-center w-full">
        {activeTab === "insert" ? (
          ...
          <div className="w-full p-4">
            <div className="relative w-full mb-6">{renderAddContent()}</div>
            {activeTab !== "welcome" && (
