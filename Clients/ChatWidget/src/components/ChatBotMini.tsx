...
const ChatBotMini: React.FC<ChatBotMiniProps> = ({
  ...
  const [formFieldsWereShown, setFormFieldsWereShown] = useState(false);
  ...
  const handleStreamToggle = useCallback(async () => {
    ...
    if (isStreaming) {
      ...
      if (formFieldsWereShown) {
        window.parent.postMessage({ type: 'HIDE_FORM_FIELDS' }, '*');
        setFormFieldsWereShown(false);
      }
  ...
  }, [agentId, isStreaming, setIsStreaming, setIsLiveKitActive, setToken, setUrl, setIsConnecting, eventBridge, apiUrl, formFieldsWereShown]);
